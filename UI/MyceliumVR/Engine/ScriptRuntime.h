...
class ScriptRuntime {
public:
    explicit ScriptRuntime(World&, VirtualFileSystem* = nullptr);
    ~ScriptRuntime();
...
private:
    ...
    JS::ThrowCompletionOr<JS::Value> call_if_function(Utf16FlyString const& name, JS::Value argument = JS::js_undefined());
    ...
    bool m_reported_missing_update { false };
