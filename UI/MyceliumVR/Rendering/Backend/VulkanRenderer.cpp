...
static ErrorOr<void> ensure_vulkan_resources(VulkanContext const& ctx, VulkanRenderer::Impl& impl)
{
    ...
    if (impl.panel_pipeline.pipeline == VK_NULL_HANDLE && impl.panel_vertex_shader_bytecode.has_value()) {
...
static ErrorOr<void> validate_compiled_frame_graph(RenderGraph const& graph, VulkanRenderer::Impl const& impl, World const* world)
{
    ...
    bool expect_cull = world && !impl.geometry_pass->draw_groups().is_empty();
...
static ErrorOr<void> draw_frame(VulkanContext const& ctx, VulkanRenderer::Impl& impl, MyceliumVR::CameraState const& camera_state, World const* world = nullptr, VulkanRenderer::OverlayView const* overlay_view = nullptr)
{
    ...
    if (overlay_view && impl.ui_panel_pass) {
        impl.ui_panel_pass->set_overlay_view({ overlay_view->pixels, overlay_view->width, overlay_view->height });
    }

    impl.geometry_pass->set_world(world);
    impl.geometry_pass->set_scene_light(impl.scene_light);
    impl.geometry_pass->set_camera_position(camera_state.position);
    ...
    auto pc = make_view_projection_constants(impl.sc.extent, camera_state);
    auto view_matrix = make_view_matrix(camera_state);

    // Projection focal-length parameters (must match make_perspective_matrix).
    static constexpr float k_fov = 1.0471976f;
    float safe_height = impl.sc.extent.height > 0 ? static_cast<float>(impl.sc.extent.height) : 1.0f;
    float aspect_ratio = static_cast<float>(impl.sc.extent.width) / safe_height;
    float fl = 1.0f / __builtin_tanf(k_fov * 0.5f);
    ...
    if (impl.cluster_light_assign_pass) {
        ...
        impl.cluster_light_assign_pass->set_projection_params(0.1f, 100.0f, proj_x, proj_y);
    }

    impl.geometry_pass->set_cluster_descriptor_set(impl.cluster_graphics_dset);
    impl.geometry_pass->set_cluster_params(0.1f, 100.0f);
    ...
    if (impl.ui_panel_pass->panel_texture().image != VK_NULL_HANDLE) {
        panel_texture = graph.import_texture(make_imported_texture(impl.names.ui_panel_texture, impl.ui_panel_pass->panel_texture().image, impl.ui_panel_pass->panel_texture().view, VK_FORMAT_R8G8B8A8_UNORM, { impl.ui_panel_pass->panel_texture().width, impl.ui_panel_pass->panel_texture().height }, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, impl.ui_panel_pass->panel_texture_initial_state()));
    }
    TextureHandle overlay_texture {};
    if (impl.ui_panel_pass->overlay_texture().image != VK_NULL_HANDLE) {
        overlay_texture = graph.import_texture(make_imported_texture(impl.names.ui_overlay_texture, impl.ui_panel_pass->overlay_texture().image, impl.ui_panel_pass->overlay_texture().view, VK_FORMAT_R8G8B8A8_UNORM, { impl.ui_panel_pass->overlay_texture().width, impl.ui_panel_pass->overlay_texture().height }, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, impl.ui_panel_pass->overlay_texture_initial_state()));
    }
    ...
    BufferHandle panel_texture_staging {};
    if (impl.ui_panel_pass->has_panel_texture_upload())
        panel_texture_staging = graph.import_buffer(make_imported_buffer(impl.names.ui_panel_texture_staging, impl.ui_panel_pass->panel_texture_staging_buffer().buffer, impl.ui_panel_pass->panel_texture_staging_buffer().size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceState::TransferSrc));
    BufferHandle overlay_texture_staging {};
    if (impl.ui_panel_pass->has_overlay_texture_upload())
        overlay_texture_staging = graph.import_buffer(make_imported_buffer(impl.names.ui_overlay_texture_staging, impl.ui_panel_pass->overlay_texture_staging_buffer().buffer, impl.ui_panel_pass->overlay_texture_staging_buffer().size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ResourceState::TransferSrc));

    bool depth_prepass_active = false;
    if (impl.depth_prepass && world && !impl.geometry_pass->draw_groups().is_empty()) {
    ...
    CallbackGraphPass ui_upload_pass(
        ...
        [&](RenderPassBuilder& builder) {
            builder.set_queue_preference(QueueKind::Transfer);
            if (panel_texture_staging.value != 0 && panel_texture.value != 0) {
                builder.read_transfer(panel_texture_staging);
                builder.write_transfer(panel_texture);
            }
            if (overlay_texture_staging.value != 0 && overlay_texture.value != 0) {
                builder.read_transfer(overlay_texture_staging);
                builder.write_transfer(overlay_texture);
            }
    ...
    auto update_post_descriptor_set = [&](VkDescriptorSet descriptor_set, VkImageView source_view, VkImageView auxiliary_view) {
        VkDescriptorImageInfo image_infos[2] {
            { impl.post_sampler, source_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { impl.post_sampler, auxiliary_view != VK_NULL_HANDLE ? auxiliary_view : source_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        };
    ...
    auto make_post_pass = [&](String pass_name, TextureHandle input_a, TextureHandle input_b, TextureHandle output, PostProcessMode mode, VkDescriptorSet descriptor_set) {
        auto captured_input_a = input_a;
        auto captured_input_b = input_b;
        auto captured_output = output;
        auto captured_mode = mode;
        auto captured_descriptor_set = descriptor_set;
        return CallbackGraphPass(
            move(pass_name),
            [captured_input_a, captured_input_b, captured_output](RenderPassBuilder& builder) {
                builder.read_sampled(captured_input_a);
                if (captured_input_b.value != 0)
                    builder.read_sampled(captured_input_b);
                VkClearValue clear {};
                builder.write_color(captured_output, { VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE, clear });
            },
            [&, captured_input_a, captured_input_b, captured_output, captured_mode, captured_descriptor_set](VkCommandBuffer cmd, RenderGraph& graph) {
                auto extent = graph.get_texture_extent(captured_output);
                VkViewport viewport {
                    .x = 0.0f,
                    .y = 0.0f,
                ...
                update_post_descriptor_set(captured_descriptor_set,
                    graph.get_texture_view(captured_input_a),
                    captured_input_b.value != 0 ? graph.get_texture_view(captured_input_b) : VK_NULL_HANDLE);
                ...
                push_constants.threshold = 1.1f;
                push_constants.bloom_strength = 0.18f;
                push_constants.mode = static_cast<u32>(captured_mode);

                vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, impl.post_pipeline.pipeline);
                vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, impl.post_pipeline.pipeline_layout, 0, 1, &captured_descriptor_set, 0, nullptr);
                vkCmdPushConstants(cmd, impl.post_pipeline.pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PostProcessPushConstants), &push_constants);
...

    {
        ...
        if (world && !impl.geometry_pass->draw_groups().is_empty()) {
            {
                if (impl.shadow_quality > 0) {
        ...
        CallbackGraphPass bloom_threshold_pass(MUST(String::from_utf8(""sv)), [](RenderPassBuilder&) {}, [](VkCommandBuffer, RenderGraph&) {});
        CallbackGraphPass bloom_blur_h_pass(MUST(String::from_utf8(""sv)), [](RenderPassBuilder&) {}, [](VkCommandBuffer, RenderGraph&) {});
        CallbackGraphPass bloom_blur_v_pass(MUST(String::from_utf8(""sv)), [](RenderPassBuilder&) {}, [](VkCommandBuffer, RenderGraph&) {});
        CallbackGraphPass composite_pass(MUST(String::from_utf8(""sv)), [](RenderPassBuilder&) {}, [](VkCommandBuffer, RenderGraph&) {});
        {
            ZoneScopedN("Frame/AddPasses/PostPasses");
            bloom_threshold_pass = make_post_pass(impl.names.bloom_threshold_name, scene_color, {}, bloom_threshold, PostProcessMode::Threshold, impl.post_descriptor_sets[0]);
            bloom_blur_h_pass = make_post_pass(impl.names.bloom_blur_h_name, bloom_threshold, {}, bloom_blur_h, PostProcessMode::BlurHorizontal, impl.post_descriptor_sets[1]);
            bloom_blur_v_pass = make_post_pass(impl.names.bloom_blur_v_name, bloom_blur_h, {}, bloom_blur_v, PostProcessMode::BlurVertical, impl.post_descriptor_sets[2]);
            composite_pass = make_post_pass(impl.names.composite_name, scene_color, bloom_blur_v, composite_color, PostProcessMode::Composite, impl.post_descriptor_sets[3]);
            graph.add_pass(bloom_threshold_pass);
            graph.add_pass(bloom_blur_h_pass);
            graph.add_pass(bloom_blur_v_pass);
